...
if ( ! class_exists( 'c2c_RestrictUsernames_Plugin_048' ) ) :

abstract class c2c_RestrictUsernames_Plugin_048 {
	...
	protected function __construct( $version, $id_base, $author_prefix, $file, $plugin_options = array() ) {
		...
		if ( ! file_exists( $file ) ) {
			die( sprintf( __( 'Invalid file specified for C2C_Plugin: %s', 'restrict-usernames' ), $file ) );
		}
	 ...
	 * @since 036
	 */
	public function __clone() { _doing_it_wrong( __FUNCTION__, __( 'Something went wrong.', 'restrict-usernames' ), '036' ); }
	 ...
	 * @since 036
	 */
	public function __wakeup() { _doing_it_wrong( __FUNCTION__, __( 'Something went wrong.', 'restrict-usernames' ), '036' ); }
	...
	public function options_page_description( $localized_heading_text = '' ) {
		...
		if ( ! $this->disable_contextual_help ) {
			echo '<p class="see-help">' . __( 'See the "Help" link to the top-right of the page for more help.', 'restrict-usernames' ) . "</p>\n";
		}
	}
	...
	public function sanitize_inputs( $inputs ) {
		...
		if ( isset( $_POST['Reset'] ) ) {
			...
			add_settings_error( 'general', 'settings_reset', __( 'Settings reset.', 'restrict-usernames' ), 'updated' );
			unset( $_POST['Reset'] );
		} else {
			...
			foreach ( $option_names as $opt ) {
				if ( !isset( $inputs[ $opt ] ) ) {
					if ( $this->config[ $opt ]['input'] == 'checkbox' ) {
						...
						$msg = sprintf( __( 'A value is required for: "%s"', 'restrict-usernames' ), $this->config[ $opt ]['label'] );
						...
						switch ( $this->config[ $opt ]['datatype'] ) {
							...
							case 'int':
								if ( ! empty( $val ) && ( ! is_numeric( $val ) || ( intval( $val ) != round( $val ) ) ) ) {
									$msg = sprintf( __( 'Expected integer value for: %s', 'restrict-usernames' ), $this->config[ $opt ]['label'] );
									$error = true;
									$val = '';
								}
								break;
	...
	public function verify_config() {
		...
		foreach ( $this->required_config as $config ) {
			if ( empty( $this->$config ) ) {
				die( sprintf( __( "The plugin configuration option '%s' must be supplied.", 'restrict-usernames' ), $config ) );
			}
		}
	...
	protected function load_textdomain() {
		load_plugin_textdomain( 'restrict-usernames' );
	}
	...
	public function contextual_help( $contextual_help, $screen_id, $screen = null ) {
		...
		$help = '<h3>' . __( 'More Plugin Help', 'restrict-usernames' ) . '</h3>';
		$help .= '<p class="more-help">';
		$help .= '<a title="' . esc_attr( sprintf( __( 'More information about %1$s %2$s', 'restrict-usernames' ), $this->name, $this->version ) ) .
			'" class="thickbox" href="' . $help_url . '">' . __( 'Click for more help on this plugin', 'restrict-usernames' ) . '</a>' .
			__( ' (especially check out the "Other Notes" tab, if present)', 'restrict-usernames' );
		$help .= ".</p>\n";
		return $help;
	}
...
HTML;
	...
	public function help_tabs_content( $screen ) {
		$screen->add_help_tab( array(
			...
			'title'   => __( 'More Help', 'restrict-usernames' ),
	...
	public function plugin_action_links( $action_links ) {
		$settings_link = '<a href="' . $this->settings_page . '.php?page='.$this->plugin_basename.'">' . __( 'Settings', 'restrict-usernames' ) . '</a>';
	...
	public function donate_link( $links, $file ) {
		if ( $file == $this->plugin_basename ) {
			...
			$donation_url .= urlencode( sprintf( __( 'Donation for coffee2code plugin: %s', 'restrict-usernames' ), $this->name ) );
			$title         = __( 'Coffee fuels my coding.', 'restrict-usernames' );
			$links[] = '<a href="' . esc_url( $donation_url ) . '" title="' . esc_attr( $title ) . '">' . __( 'Donate', 'restrict-usernames' ) . '</a>';
		}
		return $links;
	}
...
//				echo '<div class="hr">&nbsp;</div>' . "\n";
	...
	public function options_page() {
		...
		echo '<input type="submit" name="Submit" class="button-primary" value="' . esc_attr__( 'Save Changes', 'restrict-usernames' ) . '" />' . "\n";
		echo '<input type="submit" name="Reset" class="button" value="' . esc_attr__( 'Reset Settings', 'restrict-usernames' ) . '" />' . "\n";
		echo '</form>' . "\n";

		do_action( $this->get_hook( 'after_settings_form' ), $this );

		echo '<div id="c2c" class="wrap"><div>' . "\n";
		$c2c = '<a href="http://coffee2code.com" title="coffee2code.com">' . __( 'Scott Reilly, aka coffee2code', 'restrict-usernames' ) . '</a>';
		echo sprintf( __( 'This plugin brought to you by %s.', 'restrict-usernames' ), $c2c );
		echo '<span><a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=6ARCFJ9TX3522" title="' . esc_attr__( 'Please consider a donation', 'restrict-usernames' ) . '">' .
		__( 'Did you find this plugin useful?', 'restrict-usernames' ) . '</a></span>';
		echo '</div>' . "\n";
